...
export class BlockedUsersController {
  ...
  @Post()
  async blockUser(@Req() req: any, @Body('blockedUserId') blockedUserId: string) {
    const userId = req.user.sub;
    return this.blockedUsersService.block(userId, blockedUserId);
  }

  @Post('unblock')
  async unblockUser(@Req() req: any, @Body('blockedUserId') blockedUserId: string) {
    const userId = req.user.sub;
    return this.blockedUsersService.unblock(userId, blockedUserId);
  }

  @Get('check/:targetUserId')
  async checkBlocked(@Req() req: any, @Param('targetUserId') targetUserId: string) {
    const userId = req.user.sub;
    const blocked = await this.blockedUsersService.isBlockedByUser(userId, targetUserId);
    return { blocked };
  }
